Shader "Planet/CityLight"
{
    Properties
    {
        ...
        cityLightTexture ("city light texture", 2D) = "black"{}
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float4 col = max(tex2Dlod(cityLightTexture,float4(uv.x,uv.y,0.0,0.0)),0.0);
                // col.x = 1.0;
                // col.y = 1.0;
                // col.z = 1.0;
                float4 translucentLight;
                float4 translucentGround;
                IngAirFogPropInfo infoCamera = getIngAirFogPropInfoByRelPos(pos,eye,sun,distance(fargPos,pos));
                getTranslucents(infoCamera,translucentLight,translucentGround);
                // col.xz += float2(0.08,0.2) * col.w;
                col.xyz *= clamp((1.0 - bright(translucentLight) * 16.0),0.0,1.0);
