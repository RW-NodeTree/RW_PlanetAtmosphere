    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            f2bColor frag (v2f i)
            {
                ...
                if(cloudPosA.w == 0) discard;
                float4 clipSpacePos = UnityWorldToClipPos(cloudPosA.xyz + i.worldSpaceZeroPoint);
                o.reflection.xyz = max(cloudColorA.xyz * _LightColor0.xyz * cloudColorA.w * refraction,0) * saturate(dot(normalize(cloudPosA.xyz) * diffusePower,sun));
        ...
        Pass
        {
            ...
            f2bColor frag (v2f i)
            {
                ...
                if(cloudPosB.w == 0) discard;
                float4 clipSpacePos = UnityWorldToClipPos(cloudPosB.xyz + i.worldSpaceZeroPoint);
                o.reflection.xyz = max(cloudColorB.xyz * _LightColor0.xyz * cloudColorB.w * refraction, 0) * saturate(dot(normalize(cloudPosB.xyz) * diffusePower,sun));
