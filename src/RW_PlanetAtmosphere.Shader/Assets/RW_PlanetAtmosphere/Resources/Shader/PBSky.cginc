...
float4 translucentFromLUT(float2 ah)
{
    ...
    return saturate(tex2Dlod(translucentLUT,float4(ah.x,ah.y,0.0,0.0))) * valid.x;
    // return saturate(tex2Dlod(translucentLUT,float4(ah.x,ah.y,0.0,0.0)));
}
...
float4 inSunLightFromLUT(float2 ah)
{
    ...
    // return saturate(tex2Dlod(inSunLightLUT,float4(ah.x,ah.y,0.0,0.0)));
}
...
float4 outSunLightFromLUT(float2 ad)
{
    ...
    // return saturate(tex2Dlod(outSunLightLUT,float4(ad.x,ad.y,0.0,0.0)));
}
